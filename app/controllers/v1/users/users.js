...
if (order && !['asc', 'desc'].includes(order)) {
    return res.badRequest({message:MESSAGES.apiErrorStrings.DATA_IS_INVALID(order)})
}

    if(searchInput) 
    {
                users = users.filter(u => u.name.toLowerCase().includes(searchInput) || u.email.toLowerCase().includes(searchInput));
    }
    ...
    if(sort)
    {
        if(sort === 'name') 
        { 
                users.sort((a, b) => order === 'desc' ? b.name.localeCompare(a.name) : a.name.localeCompare(b.name) ); 
        }
    }
    res.success(users);
});
...
if (isNaN(ID)) {
    ...
    user ? res.success(user) 
         : res.notFound({message:MESSAGES.apiErrorStrings.DATA_NOT_EXISTS("user")});
})
...
const { name, email,role} = req.body;
        ...
        if (!name || !email || !role) {
            return res.badRequest({ message:MESSAGES.apiErrorStrings.DATA_IS_INVALID("user data")});
        }
...
const handleDeleteUser = handleAsync(async(req,res)=>{
    const userId = parseInt(req.params.id);
        ...
        res.success({ message:MESSAGES.apiSuccessStrings.DELETE(`User with userId ${userId}`)});
})
